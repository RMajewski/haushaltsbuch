...
public class TestDbChange {
	/**
	 * Speichert die Instanz von DbChange
	 */
	private DbChange _test;
	...
	public void testDbNewInheritedAction() {
		assertEquals("haushaltsbuch.actions.Action",
				DbChange.class.getSuperclass().getName());
	}
	...
	public void testCommandIsRight() {
		assertEquals(DbChange.COMMAND, _test.getValue(Action.ACTION_COMMAND_KEY));
	}
	...
	public void testNameIsRight() {
		assertEquals("Ändern", _test.getValue(Action.NAME));
	}
	...
	public void testCommandStringIsRight() {
		assertEquals("DatabaseChange", DbChange.COMMAND);
	}
}
